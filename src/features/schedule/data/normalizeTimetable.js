...
export function normalizeTimetable(scheduleId, rows) {
  // Adapt database rows to the calendar model at the data boundary.
  ...
  rawEvents.forEach((item, idx) => {
    ...
    const start = normalizeHm(normalizeDbTime(item?.start_time));
    if (!start) return;

    const end = addMinutesToHm(start, item?.duration_min);
    if (!end) return;
    ...
    const status = String(item?.status || "").trim() || "aktywne";
    ...
    if (!merged.has(eventKey)) {
      merged.set(eventKey, {
        id: `${scheduleId}-${idx + 1}`,
        event_id: String(item?.id || "").trim() || undefined,
        subj: subjCode,
        title: subjectName,
        type: eventType,
        status,
