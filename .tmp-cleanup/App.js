...
import FAQ from "./FAQ";
import { useSettings, usePersistSettings } from "../features/settings/useSettings";
...
export default function App() {
  ...
  }), [schedule.schedule, schedule.studentGroups, schedule.activeExternalSelections,
    schedule.loadedTimetables, view.hideLectures, view.showAll]);
  const weekEvents = useMemo(() => mergeEvents({
    ...eventOptions,
    weekStart: navigation.viewedWeekStart,
    addedEvents: myPlan.addedEventsByWeek[toIsoDate(navigation.viewedWeekStart)] || [],
  }), [eventOptions, navigation.viewedWeekStart, myPlan.addedEventsByWeek]);
  const dayEvents = useMemo(() => mergeEvents({
    ...eventOptions,
    weekStart: navigation.selectedDayWeekStart,
    addedEvents: myPlan.addedEventsByWeek[toIsoDate(navigation.selectedDayWeekStart)] || [],
    ...
    <div className="min-h-screen bg-black text-white p-6 pb-[calc(140px+env(safe-area-inset-bottom))]">
      <FloatingMenu
        ...
        scheduleState={{
          ...
          onRefreshSchedule: async () => { await schedule.handleRefreshSchedule(); myPlan.refresh(); },
      ...
      />
      {view.viewMode === "week" ? (
        <WeekView {...calendarProps} events={weekEvents} viewedWeekStart={navigation.viewedWeekStart} />
      ) : (
        <DayView {...calendarProps} key={navigation.daySelection.selection} events={dayEvents} selection={navigation.daySelection.selection} />
      )}
      <FAQ />
    </div>
  );
